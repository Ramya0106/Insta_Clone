...
sendBtn.onclick = ()=>{
    ...
    xhr.onload = ()=>{
        if(xhr.readyState == XMLHttpRequest.DONE){
            if(xhr.status == 200){
                inputField.value = ""; //once message is inserted into db then leave blank in input field
                scrollToBottom();
            }
        }
    }
...
setInterval(()=>{
    //lets start ajax
    let xhr = new XMLHttpRequest(); //creating XML object;
    xhr.open("POST", "php/get-chat.php", true); //using get method bcoz we r receiving data
    xhr.onload = ()=>{
        if(xhr.readyState === XMLHttpRequest.DONE){
            if(xhr.status === 200){
...
function scrollToBottom(){ //scroll to bottom after msg send
